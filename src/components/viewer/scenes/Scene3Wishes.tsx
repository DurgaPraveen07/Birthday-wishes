...
export const Scene3Wishes: React.FC<Props> = ({ theme, wishes }) => {
  ...
  return (
    <div
      id="scene-3"
      className={`min-h-screen w-full flex flex-col justify-center items-center p-6 text-center relative overflow-hidden bg-gradient-to-b ${theme.bgGradient} snap-start py-16`}
    >
      <div className="max-w-2xl w-full space-y-8 z-10">
        <div className="space-y-2">
          <span className={`px-4 py-1.5 rounded-full border text-xs font-bold uppercase tracking-widest inline-block ${theme.badgeBg}`}>
            {theme.wishesBadge} ({wishes.length} Items)
          </span>
          <h2 className="text-3xl sm:text-5xl font-bold font-heading text-white">
            {theme.wishesSectionTitle}
          </h2>
          <p className="text-xs sm:text-sm text-slate-300">
            {theme.wishesSectionSubtitle}
          </p>
        </div>
        ...
        {allPopped && (
          ...
          >
            <span className={`px-6 py-2.5 rounded-full bg-gradient-to-r ${theme.buttonGradient} text-white font-bold text-sm shadow-xl inline-block glow-gold`}>
              All Unlocked! Keep going 💛
            </span>
          </motion.div>
        )}
      </div>
